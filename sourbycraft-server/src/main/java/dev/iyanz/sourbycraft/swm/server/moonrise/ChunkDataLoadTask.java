...
public final class ChunkDataLoadTask implements CommonLoadTask {
    ...
    private static final Logger LOGGER = LoggerFactory.getLogger(ChunkDataLoadTask.class);
    ...
    public ChunkDataLoadTask(ChunkLoadTask chunkLoadTask, final ChunkTaskScheduler scheduler, final ServerLevel world, final int chunkX,
        ...
        this.task = this.scheduler.createChunkTask(this.chunkX, this.chunkZ, () -> {
            try {
                SlimeChunk chunk = ((dev.iyanz.sourbycraft.swm.server.SlimeLevelInstance) this.world).slimeInstance.getChunk(this.chunkX, this.chunkZ);
    ...
    private ChunkAccess runOnMain(final SlimeChunk data) {
        try {
            LevelChunk chunk = ((dev.iyanz.sourbycraft.swm.server.SlimeLevelInstance) this.world).slimeInstance.createChunk(chunkX, chunkZ, data);
